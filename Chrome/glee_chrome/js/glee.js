...
var Glee = {
  ...
  setState: function(value, type) {
    ...
    // set state for bookmark. value is the bookmark no. in Glee.bookmarks
      ...
      this.description('Open bookmark (' + (value + 1)
                        + ' of '
                        + (this.bookmarks.length - 1) + '): '
                        + this.bookmarks[value].title, true);
  ...
  getEspSelector: function() {
    ...
    if (sel.length != 0)
      return sel.join(',');
    // search for any default selector defined by meta tag in current page
    else
      ...
      if (Glee.status) {
        if (!Utils.elementCanReceiveUserInput(target) || e.altKey) {
          ...
          if (e.keyCode === Glee.options.shortcutKey
              || (e.keyCode === Glee.options.tabManagerShortcutKey
                  && Glee.options.tabManager)) {
